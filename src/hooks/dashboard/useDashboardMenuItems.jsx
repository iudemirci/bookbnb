...
export function useDashboardMenuItems(isLogoutPending = false, reportCount = 0) {
  ...
  return useMemo(() => {
    const baseItems = [
      ...
      { label: t('dashboard:users'), key: 'users', icon: <Icon icon='uil:user' width={20} /> },
      {
        ...
        key: 'reports',
        icon: (
          <Badge dot={reportCount > 0}>
            <Icon icon='uil:ticket' width={20} />
          </Badge>
        ),
      },
    ];
